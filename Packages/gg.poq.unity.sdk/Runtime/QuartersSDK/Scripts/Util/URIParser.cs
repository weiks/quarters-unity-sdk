...
namespace QuartersSDK {
    public class QuartersLink {
        ...
        public static QuartersLink Create(string rawUri) {
            ...
            try {
                UrlEncodingParser parser = new UrlEncodingParser(rawUri);
                args = parser;
                query = parser.Query;
            }
            catch (Exception) {
    ...
    public class UrlEncodingParser : Dictionary<string, string> {
        ...
        public UrlEncodingParser(string queryStringOrUrl = null) {
            ...
            if (!string.IsNullOrEmpty(queryStringOrUrl)) Parse(queryStringOrUrl);
        }
            ...
            if (string.IsNullOrEmpty(query)) {
                ...
                foreach (string pair in pairs) {
                    ...
                    if (index2 > 0) {
                        ...
                        string value = pair.Substring(index2 + 1);

                        string origKey = key;
                        int val = 2;
                        while (ContainsKey(key)) key = origKey + val++;
        ...
        public override string ToString() {
            string query = string.Empty;
            foreach (string key in Keys) query += key + "=" + Uri.EscapeUriString(this[key]) + "&";
            query = query.Trim('&');
